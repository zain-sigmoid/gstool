...
if page == "Introduction":
    st.title("🛡️ Code Compliance Analyzer")
    st.markdown(
        """
    This tool analyzes your Python codebase for compliance, privacy, and licensing risks to help ensure safe, secure, and legally sound software distribution.

    ### 🔍 What it checks:
    - 📄 **License Compliance:** Detects open-source licenses in your codebase and flags non-permissive or incompatible licenses (e.g., GPL, AGPL, internal-use-only).

    - 🔐 **Data Privacy Violations:** Highlights potential misuse or exposure of sensitive data such as personally identifiable information (PII), insecure transmission (e.g., HTTP), or missing deletion mechanisms.

    - ©️ **Copyright Issues:** Scans for missing attribution, license headers, or unidentified authorship and suggests corrections to meet compliance requirements.
    ...
    if directory and os.path.isdir(directory):
        ...
        for f in findings_to_show:
            ...
            description = re.sub(
                r"Total in file\s*(\d+)",
                r"Total in file <code>\1</code>",
                description,
            )
            description = re.sub(
                r"^([A-Za-z \-]+)\s*:", r"<code>\1</code>:", description
            )
            lines = f.get("lines", "")
            rule = f.get("rule", "")
            ...
            line_list = [
                line.strip() for line in lines.split(",") if line.strip().isdigit()
            ]
            line_html = ""
            if lines:
                line_list = [l.strip() for l in lines.split(",") if l.strip().isdigit()]
                if line_list:
                    line_html = "<div class='line-tags'><strong>🔢 Lines:</strong><br>"
                    for line in line_list:
                        line_html += f"<span class='line-chip'>Line {line}</span>"
                    line_html += "</div>"
            ...
            if rule:
                compliance_html = f"""
                    ...
                    <span class="meta-label">🔖 Rule:</span> <span class="meta-value">{rule}</span><br>
                    <span class="meta-label">📂 Category:</span> <span class="meta-value">{category}</span><br>
                    <span class="meta-label">✅ Compliance:</span> <span class="meta-value">{compliance}</span>
                    </div>
                """
            card_html = f"""
