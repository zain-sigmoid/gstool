...
@dataclass
class Issue:
    category: str                # e.g., "maintainability"
    rule: str                    # e.g., "Cyclomatic Complexity"
    title: str                   # e.g., 'Function "foo" has high cyclomatic complexity (34)'
            ...
            for item in os.listdir(current_dir):
                ...
                if os.path.isfile(item_path):
                    ...
                    if item.endswith('.py') and not should_ignore_file(item):
                        ...
                        try:
                            if os.path.getsize(item_path) < 10 * 1024 * 1024:  # 10MB limit
...
def run_single_analyzer(key: str, fn, py_files: List[str], delay_ms: int = 600) -> Tuple[str, AnalyzerOutput]:
        # Optional small delay to make progress UI readable
        time.sleep(delay_ms / 1000.0)
        return key, fn(py_files)
