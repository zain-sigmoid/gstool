...
class ToolRunner:
    ...
    def _check_tool_availability(self):
        ...
        tools_to_check = [
            'pylint', 'semgrep', 'gitleaks', 'radon', 'mypy',
            'pip-audit', 'pip-licenses', 'bandit', 'safety'
        ]
    ...
    def run_tool(self, tool_name: str, args: List[str], 
                 timeout: int = 300, capture_output: bool = True,
                 cwd: Optional[str] = None, env: Optional[Dict[str, str]] = None) -> subprocess.CompletedProcess:
        """
        ...
        if not self.is_tool_available(tool_name):
            ...
            if tool_name in ['pip-audit', 'pip-licenses']:
                try:
                    ...
                    raise ToolNotFoundError(f"Tool '{tool_name}' is not available and could not be installed")
            else:
        ...
        try:
            ...
            raise ToolTimeoutError(f"Tool '{tool_name}' timed out after {timeout} seconds")
    ...
    def run_pylint(self, path: str, config_file: Optional[str] = None) -> subprocess.CompletedProcess:
        """
        Run pylint analysis.
    ...
    def run_semgrep(self, path: str, rules: Optional[List[str]] = None) -> subprocess.CompletedProcess:
        """
    ...
    def run_mypy(self, path: str, config_file: Optional[str] = None) -> subprocess.CompletedProcess:
        """
    ...
    def run_tool_with_retry(self, tool_name: str, args: List[str], 
                           max_retries: int = 2, **kwargs) -> subprocess.CompletedProcess:
        """
